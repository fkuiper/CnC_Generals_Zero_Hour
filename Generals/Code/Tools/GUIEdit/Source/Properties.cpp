...
void InitPropertiesDialog( GameWindow *window, Int x, Int y )
{
	...
	if( CNC_BitTest( window->winGetStyle(), GWS_PUSH_BUTTON ) )
		dialog = InitPushButtonPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_RADIO_BUTTON ) )
		dialog = InitRadioButtonPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_TAB_CONTROL ) )
		dialog = InitTabControlPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_CHECK_BOX ) )
		dialog = InitCheckBoxPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_SCROLL_LISTBOX ) )
		dialog = InitListboxPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_PROGRESS_BAR ) )
		dialog = InitProgressBarPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_STATIC_TEXT ) )
		dialog = InitStaticTextPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_ENTRY_FIELD ) )
		dialog = InitTextEntryPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_ALL_SLIDER ) )
		dialog = InitSliderPropertiesDialog( window );
	else if( CNC_BitTest( window->winGetStyle(), GWS_COMBO_BOX ) )
...
static void saveTextLabel( HWND edit, GameWindow *window )
{
	...
	if( CNC_BitTest( style, GWS_PUSH_BUTTON ) )
		GadgetButtonSetText( window, text );
	else if( CNC_BitTest( style, GWS_CHECK_BOX ) )
		GadgetCheckBoxSetText( window, text );
	else if( CNC_BitTest( style, GWS_RADIO_BUTTON ) )
		GadgetRadioSetText( window, text );
	else if( CNC_BitTest( style, GWS_STATIC_TEXT ) )
		GadgetStaticTextSetText( window, text );
	else if( CNC_BitTest( style, GWS_ENTRY_FIELD ) )
...
void LoadStateCombo( UnsignedInt style, HWND comboBox )
{
	...
	for( entry = imageAndColorTable; entry->stateName; entry++ )
	{
		...
		if( CNC_BitTest( entry->windowType, style ) )
		{
...
void CommonDialogInitialize( GameWindow *window, HWND dialog )
{
	...
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_ENABLED ) )
		CheckDlgButton( dialog, CHECK_ENABLED, BST_CHECKED );
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_DRAGABLE ) )
		CheckDlgButton( dialog, CHECK_DRAGABLE, BST_CHECKED );
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_HIDDEN ) )
		CheckDlgButton( dialog, CHECK_HIDDEN, BST_CHECKED );
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_NO_INPUT ) )
	...
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_BORDER ) )
		CheckDlgButton( dialog, CHECK_BORDER, BST_CHECKED );
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_IMAGE ) )
		CheckDlgButton( dialog, CHECK_IMAGE, BST_CHECKED );
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_SEE_THRU ) )
		CheckDlgButton( dialog, CHECK_SEE_THRU, BST_CHECKED );
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_WRAP_CENTERED ) )
		CheckDlgButton( dialog, CHECK_WRAP_CENTERED, BST_CHECKED );
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) )
...
static void adjustGadgetDrawMethods( Bool useImages, GameWindow *window )
{
	...
	if( TheEditor->windowIsGadget( window ) )//The below only applies to gadgets.
	{
		...
		if( useImages )
		{
			...
			else if( CNC_BitTest( style, GWS_TAB_CONTROL ) )
...
Bool SaveCommonDialogProperties( HWND dialog, GameWindow *window )
{
	...
	if( TheEditor->windowIsGadget( window ) )
		adjustGadgetDrawMethods( CNC_BitTest( window->winGetStatus(), WIN_STATUS_IMAGE ),
														 window );

																		 ...
																		 Int srcX, Int srcY )
{
	...
	if( srcY < imageHeight / 2 &&
			(srcY != 0 || CNC_BitTest( fitBits,	ImageInfo::FIT_YBORDER_TOP )) )
	{
																		...
																		Bool extendAlpha )
{
	...
	if( CNC_BitTest( image->m_status, ImageInfo::ROTATED90C ) )
	{
	...
	for( y = 0; y < imageHeight; y++ )
	{
		...
		for( x = 0; x < imageWidth; x++ )
		{
			...
			if( prevPixel == FALSE && currPixel == TRUE )
			{
				...
				//
				if( x != 0 || CNC_BitTest( image->m_fitBits, ImageInfo::FIT_XBORDER_LEFT ) )
				{
			...
			if( currPixel == TRUE )
			{
				...
				// bottom right corner
								 ...
								 CNC_BitTest( image->m_fitBits, ImageInfo::FIT_XBORDER_RIGHT ) &&
								 CNC_BitTest( image->m_fitBits, ImageInfo::FIT_YBORDER_BOTTOM ) )
					dst = (ptr - (destWidth * destBPP)) + destBPP;
																...
																ImageInfo *image )
{
	...
	if( CNC_BitTest( image->m_status, ImageInfo::ROTATED90C ) == FALSE )
	{
	...
	//
	if( CNC_BitTest( TheImagePacker->getGapMethod(), 
...
Bool TexturePage::addImage( ImageInfo *image )
{
	...
	useGutter = CNC_BitTest( TheImagePacker->getGapMethod(), 
											 ImagePacker::GAP_METHOD_GUTTER );
	useRGBExtend = CNC_BitTest( TheImagePacker->getGapMethod(),
													ImagePacker::GAP_METHOD_EXTEND_RGB );
	...
	while( triesLeft )
	{
		...
		for( y = 0; y < m_size.y; y++ )
		{
			...
			for( x = 0; x < m_size.x; x++ )
			{
				...
				if( CNC_BitTest( fitBits, ImageInfo::FIT_XBORDER_LEFT ) )
					image->m_pagePos.lo.x++;
				if( CNC_BitTest( fitBits, ImageInfo::FIT_YBORDER_TOP ) )
					image->m_pagePos.lo.y++;
				if( CNC_BitTest( fitBits, ImageInfo::FIT_XBORDER_RIGHT ) )
					image->m_pagePos.hi.x--;
				if( CNC_BitTest( fitBits, ImageInfo::FIT_YBORDER_BOTTOM ) )
					image->m_pagePos.hi.y--;
				if( CNC_BitTest( fitBits, ImageInfo::FIT_XGUTTER ) )
					image->m_pagePos.hi.x -= xGutter;
				if( CNC_BitTest( fitBits, ImageInfo::FIT_YGUTTER ) )	

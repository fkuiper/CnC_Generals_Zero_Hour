...
Bool AudioManager::shouldPlayLocally(const AudioEventRTS *audioEvent)
{
	...
	if (!CNC_BitTest(ei->m_type, (ST_PLAYER | ST_ALLIES | ST_ENEMIES | ST_EVERYONE))) {
		DEBUG_CRASH(("No player restrictions specified for '%s'. Using Everyone.\n", ei->m_audioName.str()));
		return TRUE;
	}

	if (CNC_BitTest(ei->m_type, ST_EVERYONE)) {
		return TRUE;
	}

	Player *owningPlayer = ThePlayerList->getNthPlayer(audioEvent->getPlayerIndex());

	if (CNC_BitTest(ei->m_type, ST_PLAYER) && CNC_BitTest(ei->m_type, ST_UI) && owningPlayer == NULL) {
	...
	if (CNC_BitTest(ei->m_type, ST_PLAYER))  {
		return owningPlayer == localPlayer;
	}

	if (CNC_BitTest(ei->m_type, ST_ALLIES)) {
	...
	if (CNC_BitTest(ei->m_type, ST_ENEMIES)) {

...
Player *PlayerList::getEachPlayerFromMask( PlayerMaskType& maskToAdjust )
{
	...
	for( i = 0; i < MAX_PLAYER_COUNT; i++ )
	{
		...
		player = getNthPlayer( i );
		if ( player && CNC_BitTest(player->getPlayerMask(), maskToAdjust ))
		{
...
PlayerMaskType PlayerList::getPlayersWithRelationship( Int srcPlayerIndex, UnsignedInt allowedRelationships )
{
	...
	if (CNC_BitTest(allowedRelationships, ALLOW_SAME_PLAYER))
	...
	for ( Int i = 0; i < getPlayerCount(); ++i )
	{
		...
		{
			case ENEMIES:
				if (CNC_BitTest(allowedRelationships, ALLOW_ENEMIES))
					BitSet(retVal, player->getPlayerMask());
				break;
			case ALLIES:
				if (CNC_BitTest(allowedRelationships, ALLOW_ALLIES))
					BitSet(retVal, player->getPlayerMask());
				break;
			case NEUTRAL:
				if (CNC_BitTest(allowedRelationships, ALLOW_NEUTRAL))
					BitSet(retVal, player->getPlayerMask());
				break;
		}
	}

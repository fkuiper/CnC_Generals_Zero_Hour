...
Bool ScriptConditions::evaluateNamedDiscovered(Parameter *pItemParm, Parameter* pPlayerParm)
{
	...
	if (CNC_BitTest( theObj->getStatusBits(), OBJECT_STATUS_STEALTHED) && 
		!CNC_BitTest( theObj->getStatusBits(), OBJECT_STATUS_DETECTED)) {
		return false;
	}
...
{	
	...
	for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList(); !iter.done(); iter.advance()) {
		...
		// If we are stealthed we are not visible.
		if (CNC_BitTest( pObj->getStatusBits(), OBJECT_STATUS_STEALTHED) && 
			!CNC_BitTest( pObj->getStatusBits(), OBJECT_STATUS_DETECTED)) {
			continue;
		}
	...
	for (it = pPlayer->getPlayerTeams()->begin(); it != pPlayer->getPlayerTeams()->end(); ++it) {
		for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList(); !iter.done(); iter.advance()) {
			...
			for (DLINK_ITERATOR<Object> iter = team->iterate_TeamMemberList(); !iter.done(); iter.advance()) {
				Object *pObj = iter.cur();
				if (!pObj) continue;
				if ( CNC_BitTest( pObj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) || pObj->isDisabled() )
				{

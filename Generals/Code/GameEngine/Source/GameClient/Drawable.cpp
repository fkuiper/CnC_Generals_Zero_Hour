...
void Drawable::drawHealing(const IRegion2D* healthBarRegion)
{
	...
	if( obj->isKindOf( KINDOF_NO_HEAL_ICON ) || CNC_BitTest( obj->getStatusBits(), OBJECT_STATUS_SOLD ))
		return;
...
void Drawable::drawConstructPercent( const IRegion2D *healthBarRegion )
{
	...
	if( obj == NULL || CNC_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == FALSE ||
		CNC_BitTest( obj->getStatusBits(), OBJECT_STATUS_SOLD ) == TRUE )
	{
...
void Drawable::drawHealthBar(const IRegion2D* healthBarRegion)
{
			...
			(isSelected() || (TheInGameUI && (TheInGameUI->getMousedOverDrawableID() == getID()))) )
	{
		...
		if( CNC_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) || (obj->isDisabled() && !obj->isDisabledByType(DISABLED_HELD)) )
		{
...
void Drawable::startAmbientSound(BodyDamageType dt, TimeOfDay tod)
{
	...
	if( trySound && m_ambientSound )
	{
		...
		if( info )
		{
			if(CNC_BitTest( info->m_type, ST_GLOBAL) || info->m_priority == AP_CRITICAL )
			{
				//Play it anyways.

...
UnsignedInt getPickTypesForContext( Bool forceAttackMode )
{
	...
	if (command != NULL) {
		if (CNC_BitTest( command->getOptions(), ALLOW_MINE_TARGET)) {
			types |= PICK_TYPE_MINES;
		}

		if (CNC_BitTest( command->getOptions(), ALLOW_SHRUBBERY_TARGET ) ) {
...
//		}
		...
		// For efficiency.
		if (CNC_BitTest(retVal, PICK_TYPE_MINES | PICK_TYPE_SHRUBBERY)) {
			break;
		}
	}
...
{
	if (CNC_BitTest(pickTypes, PICK_TYPE_SELECTABLE)) {
		outMask.set(KINDOF_SELECTABLE);
	}

	if (CNC_BitTest(pickTypes, PICK_TYPE_SHRUBBERY)) {
		outMask.set(KINDOF_SHRUBBERY);
	}

	if (CNC_BitTest(pickTypes, PICK_TYPE_MINES)) {
		outMask.set(KINDOF_MINE);
	}

	if (CNC_BitTest(pickTypes, PICK_TYPE_FORCEATTACKABLE)) {

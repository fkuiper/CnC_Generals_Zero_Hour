...
//-----------------------------------------------------------------------------
// CNCAnimateWindow PUBLIC FUNCTIONS /////////////////////////////////////////////
//-----------------------------------------------------------------------------
CNCAnimateWindow::CNCAnimateWindow( void )
{
	m_delay = 0;
...
void CNCAnimateWindow::setAnimData( 	ICoord2D startPos, ICoord2D endPos, 
...
void AnimateWindowManager::update( void )
{
	...
	if(m_needsUpdate)
	{
		...
		{
			CNCAnimateWindow *animWin = *it;
			if (!animWin)
			{
				DEBUG_CRASH(("There's No CNCAnimateWindow in the CNCAnimateWindow List"));
				return;
			}
...
void AnimateWindowManager::registerGameWindow(GameWindow *win, AnimTypes animType, Bool needsToFinish, UnsignedInt ms, UnsignedInt delayMs)
{
	...
	// Create a new CNCAnimateWindow class and fill in it's data.
	CNCAnimateWindow *animWin = newInstance(CNCAnimateWindow);	

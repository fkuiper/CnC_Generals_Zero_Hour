...
void ControlBar::populateCommand( Object *obj )
{
	...
	for( i = 0; i < MAX_COMMANDS_PER_SET; i++ )
	{
		...
		else
		{
			...
			if(CNC_BitTest( commandButton->getOptions(), SCRIPT_ONLY ) )
			{
				m_commandWindows[ i ]->winHide( TRUE );
				continue;
			}
...
void ControlBar::updateContextCommand( void )
{
	...
	for( i = 0; i < MAX_COMMANDS_PER_SET; i++ )
	{
		...
		//
		if(CNC_BitTest( command->getOptions(), CHECK_LIKE ))
		{

			// sanity, check like commands should have windows that are check like as well
			DEBUG_ASSERTCRASH(CNC_BitTest( win->winGetStatus(), WIN_STATUS_CHECK_LIKE ),
																												...
																												Bool forceDisabledEvaluation ) const
{
	...
	if (disabled
			&& CNC_BitTest(command->getOptions(), IGNORES_UNDERPOWERED)
	...
	if(CNC_BitTest( command->getOptions(), NEED_UPGRADE ) )
	{
	...
	if( pu && pu->firstProduction() && CNC_BitTest( command->getOptions(), NOT_QUEUEABLE ) )
	{
...
/// and re-target the firewall without having to stop or move in-betwen.. Thanks for reading 
		...
		{
			if( !CNC_BitTest( command->getOptions(), OPTION_ONE ) )
			{
				return COMMAND_AVAILABLE;
			}

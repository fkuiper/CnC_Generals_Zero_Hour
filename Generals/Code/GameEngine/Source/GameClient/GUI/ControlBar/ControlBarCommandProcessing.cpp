																							...
																							GadgetGameMessage gadgetMessage )
{
	...
	//rethink it.
	if( obj && CNC_BitTest( commandButton->getOptions(), SINGLE_USE_COMMAND ) )
	{
	...
	if(CNC_BitTest( commandButton->getOptions(), COMMAND_OPTION_NEED_TARGET ) )
	{
	...
	else switch( commandButton->getCommandType() )
	{
		...
		case GUI_COMMAND_EVACUATE:
		{
			...
			if (CNC_BitTest(commandButton->getOptions(), NEED_TARGET_POS) == FALSE) {

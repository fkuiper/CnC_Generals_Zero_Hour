...
static void computeTotalHeight( GameWindow *window )
{
	...
	for( i=0; i<list->endPos; i++ )
	{
		...
		for (Int j = 0; j < list->columns; j++)
		{
			...
			{
				if(CNC_BitTest( window->winGetStatus(), WIN_STATUS_ONE_LINE ) == TRUE )
				{
...
//	WinInstanceData *instData = window->winGetInstanceData();
	...
	if (CNC_BitTest( window->winGetStatus(), WIN_STATUS_ONE_LINE ) == FALSE )
...
//			Int mousex = mData1 & 0xFFFF;
		...
		case GWM_MOUSE_ENTERING:
		{
			...
			if(CNC_BitTest( instData->getStyle(), GWS_MOUSE_TRACK ) )
			{
				...
				CNC_BitTest( instData->m_state, WIN_STATE_HILITED );
				TheWindowManager->winSendSystemMsg( window->winGetOwner(), 
																						GBM_MOUSE_ENTERING,
																						(WindowMsgData)window, 
																						0 );
	...
	switch( msg )
	{
		...
		case GLM_SET_SELECTION:
		{
			...
			else
			{
				...
				if( parent && CNC_BitTest( parent->winGetStyle(), GWS_COMBO_BOX ) )
				{
					TheWindowManager->winSendSystemMsg( window->winGetOwner(), 
																						GLM_SELECTED,
																						(WindowMsgData)window, 
...
UnicodeString GadgetListBoxGetTextAndColor( GameWindow *listbox, Color *color, Int row, Int column)
{
	*color = 0;
	...
	if(CNC_BitTest( listbox->winGetStyle(), GWS_SCROLL_LISTBOX ) == FALSE )
		return UnicodeString::TheEmptyString;
	TextAndColor tAndC;
...
void GadgetListboxCreateScrollbar( GameWindow *listbox )
{
	...
	if(CNC_BitTest( listbox->winGetStyle(), GWS_MOUSE_TRACK ) )

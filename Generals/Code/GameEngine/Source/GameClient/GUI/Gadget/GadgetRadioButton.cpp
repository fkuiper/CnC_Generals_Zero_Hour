												...
												GameWindow *except )
{
	...
	//
	if( window != except && CNC_BitTest( window->winGetStyle(), GWS_RADIO_BUTTON ) )
	{
														 ...
														 WindowMsgData mData1, WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_LEFT_UP:
		{
			...
			if(CNC_BitTest( instData->getState(), WIN_STATE_SELECTED ) == FALSE )
			{
				RadioButtonData *radioData = (RadioButtonData *)window->winGetUserData();
			...
			else if(CNC_BitTest( instData->getState(), WIN_STATE_HILITED ) == FALSE )
			{

																						...
																						WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_MOUSE_ENTERING:
		{
			...
			if(window->winGetParent() && CNC_BitTest(window->winGetParent()->winGetStyle(),GWS_HORZ_SLIDER) )
			{
		...
		case GWM_MOUSE_LEAVING:
		{
			...
			//
			if(CNC_BitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
		...
		case GWM_LEFT_DOWN:
		{
			...
			if(CNC_BitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) )
			{
				...
				if(CNC_BitTest( instData->m_state, WIN_STATE_SELECTED ) )
					BitClear( instData->m_state, WIN_STATE_SELECTED );
				else
		...
		case GWM_LEFT_UP:
		{
			...
			//
			if(CNC_BitTest( instData->getState(), WIN_STATE_SELECTED ) &&
				CNC_BitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
			{
		...
		case GWM_RIGHT_DOWN:
		{
			...
			if(CNC_BitTest( instData->getStatus(), WIN_STATUS_RIGHT_CLICK ) )
			{
		...
		case GWM_CHAR:
		{
			switch( mData1 )
			{
				...
				case KEY_SPACE:
				{
					...
					if(CNC_BitTest( mData2, KEY_STATE_UP ) )
					{
				...
				case KEY_TAB:
				{
					...
					if(CNC_BitTest( mData2, KEY_STATE_DOWN ) )
...
void GadgetCheckLikeButtonSetVisualCheck( GameWindow *g, Bool checked )
{
	...
	if(CNC_BitTest( g->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
	{
...
Bool GadgetCheckLikeButtonIsChecked( GameWindow *g )
{
	...
	return CNC_BitTest( instData->m_state, WIN_STATE_SELECTED );

...
//			m_buildToolTipLayout = NULL;
	...
	if(!cmdButton)
		return;
	if(CNC_BitTest(cmdButton->winGetStyle(), GWS_PUSH_BUTTON))
	{
	...
	else
	{
		...
		if(!CNC_BitTest(cmdButton->winGetStyle(), GWS_USER_WINDOW) && !CNC_BitTest(cmdButton->winGetStyle(), GWS_STATIC_TEXT))
			return;
...
void ControlBar::repopulateBuildTooltipLayout( void )
{
	if(!prevWindow || !m_buildToolTipLayout)
		return;
	if(!CNC_BitTest(prevWindow->winGetStyle(), GWS_PUSH_BUTTON))
		return;

												...
												UnsignedInt mouse)
{
	...
	else
	{
		...
		else if(CNC_BitTest(battleHonor, BATTLE_HONOR_STREAK_10))
			TheMouse->setCursorTooltip( TheGameText->fetch("TOOLTIP:BattleHonorStreak10"), -1, NULL, tooltipWidth );
		else if(CNC_BitTest(battleHonor, BATTLE_HONOR_STREAK_25))
		...
		else if(CNC_BitTest(battleHonor, BATTLE_HONOR_BLITZ5))
		...
		else if(CNC_BitTest(battleHonor, BATTLE_HONOR_OFFICERSCLUB))
...
static void populateBattleHonors(const PSPlayerStats& stats, Int battleHonors, Int gamesInRow, Int lastGen, Int challenge, GameWindow *list)
{
	...
	InsertBattleHonor(list, TheMappedImageCollection->findImageByName("HonorAirWing"), CNC_BitTest(battleHonors, BATTLE_HONOR_AIR_WING),
		BATTLE_HONOR_AIR_WING, row, column);
	InsertBattleHonor(list, TheMappedImageCollection->findImageByName("HonorBattleTank"), CNC_BitTest(battleHonors, BATTLE_HONOR_BATTLE_TANK),
		BATTLE_HONOR_BATTLE_TANK, row, column);
	InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Apocalypse"), CNC_BitTest(battleHonors, BATTLE_HONOR_APOCALYPSE),
		BATTLE_HONOR_APOCALYPSE, row, column);

	if (CNC_BitTest(battleHonors, BATTLE_HONOR_BLITZ5))
	{
		InsertBattleHonor(list, TheMappedImageCollection->findImageByName("HonorBlitz5"), TRUE,
			BATTLE_HONOR_BLITZ5, row, column);
	}
	else if (CNC_BitTest(battleHonors, BATTLE_HONOR_BLITZ10))
	{
	...
	//insertBattleHonor(list, TheMappedImageCollection->findImageByName("Endurance"), CNC_BitTest(battleHonors, BATTLE_HONOR_ENDURANCE),
		//BATTLE_HONOR_ENDURANCE, row, column);

	/*
	InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Campaign_USA"), CNC_BitTest(battleHonors, BATTLE_HONOR_CAMPAIGN_USA),
		BATTLE_HONOR_CAMPAIGN_USA, row, column);
	InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Campaign_China"), CNC_BitTest(battleHonors, BATTLE_HONOR_CAMPAIGN_CHINA),
		BATTLE_HONOR_CAMPAIGN_CHINA, row, column);
	InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Campaign_GLA"), CNC_BitTest(battleHonors, BATTLE_HONOR_CAMPAIGN_GLA),
...
Int CalculateRank( const PSPlayerStats& stats )
{
	...
	if(CNC_BitTest(stats.battleHonors, BATTLE_HONOR_CAMPAIGN_USA | BATTLE_HONOR_CAMPAIGN_CHINA |BATTLE_HONOR_CAMPAIGN_GLA))
	{

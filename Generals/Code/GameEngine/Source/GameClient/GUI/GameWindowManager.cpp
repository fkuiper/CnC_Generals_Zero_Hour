...
Bool GameWindowManager::isEnabled( GameWindow *win )
{
	...
	if(CNC_BitTest( win->m_status, WIN_STATUS_ENABLED ) == FALSE )
	{
		return FALSE;
	}
...
Bool GameWindowManager::isHidden( GameWindow *win )
{
	...
	if(CNC_BitTest( win->m_status, WIN_STATUS_HIDDEN ))
	{
		return TRUE;
	}
																					...
																					WindowMsgData mData2 )
{
	...
	if( msg != GWM_DESTROY && CNC_BitTest( window->m_status, WIN_STATUS_DESTROYED ) )
		return MSG_IGNORED;
...
Int GameWindowManager::winSetFocus( GameWindow *window )
{
	...
	if( window )
		if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_NO_FOCUS) )
			return 0;
		...
		if( m_grabWindow )
		{
			...
			switch( msg )
			{
				...
				case GWM_LEFT_UP:
				{
					...
					else if( CNC_BitTest( m_grabWindow->m_status, WIN_STATUS_DRAGABLE ))
					{
						winSendInputMsg( m_grabWindow, GWM_LEFT_UP, packedMouseCoords, 0 );
					}
				...
				case GWM_LEFT_DRAG:
				{
					...
					if( CNC_BitTest( m_grabWindow->m_status, WIN_STATUS_DRAGABLE ) )
					{
		...
		else
		{
			...
			else
			{
				...
				for( window = m_windowList; window; window = window->m_next )
				{
					...
					if( CNC_BitTest( window->m_status, WIN_STATUS_ABOVE ) &&
							!CNC_BitTest( window->m_status, WIN_STATUS_HIDDEN ) &&
							...
							mousePos->y <= window->m_region.hi.y)
					{
						...
						if( CNC_BitTest( window->m_status, WIN_STATUS_ENABLED ) )
						{
				...
				if( window == NULL )
				{
					...
					for( window = m_windowList; window; window = window->m_next )
					{
						...
						if( CNC_BitTest( window->m_status, WIN_STATUS_BELOW ) &&
								!CNC_BitTest( window->m_status, WIN_STATUS_HIDDEN ) &&
								mousePos->x >= window->m_region.lo.x &&
								mousePos->x <= window->m_region.hi.x &&
								mousePos->y >= window->m_region.lo.y &&
			...
			if( window )
				if( CNC_BitTest( window->m_status, WIN_STATUS_NO_INPUT ) )
				{
					if(window->winGetParent() && CNC_BitTest( window->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX ))
						window = window->winGetParent();
					else
						window = NULL;
				}
...
//							WinActivate( tempWin );
			...
			if( tooltipsOn )
			{
//				if( toolTipWindow && toolTipWindow->winGetParent() && CNC_BitTest( toolTipWindow->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX ))
...
Int GameWindowManager::drawWindow( GameWindow *window )
{
	...
	if( CNC_BitTest( window->m_status, WIN_STATUS_HIDDEN ) == FALSE )
	{

		if( !CNC_BitTest( window->m_status, WIN_STATUS_SEE_THRU ) && window->m_draw )
		...
		if( CNC_BitTest( window->winGetStyle(), GWS_SCROLL_LISTBOX ) )
			if( CNC_BitTest( window->m_status, WIN_STATUS_BORDER ) == TRUE &&
					!CNC_BitTest( window->m_status, WIN_STATUS_SEE_THRU ) )
...
Int GameWindowManager::winDestroy( GameWindow *window )
{
	...
	if( CNC_BitTest( window->m_status, WIN_STATUS_DESTROYED ) )
		return WIN_ERR_OK;
																										 ...
																										 Bool defaultVisual )
{
	...
	if( CNC_BitTest( instData->getStyle(), GWS_PUSH_BUTTON ) == FALSE )
	{
	...
	//
	if( CNC_BitTest( button->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( CNC_BitTest( instData->getStyle(), GWS_CHECK_BOX ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( CNC_BitTest( checkbox->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( CNC_BitTest( instData->getStyle(), GWS_RADIO_BUTTON ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( CNC_BitTest( radioButton->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	if( CNC_BitTest( instData->getStyle(), GWS_TAB_CONTROL ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( CNC_BitTest( tabControl->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
  ...
  ListboxData *listboxData;
	...
	// we MUST have a push button style window to do this
	if( CNC_BitTest( instData->getStyle(), GWS_SCROLL_LISTBOX ) == FALSE )
	{
  ...
  listbox->winSetUserData( listboxData );
	...
	if( CNC_BitTest( listbox->winGetStatus(), WIN_STATUS_IMAGE ))
...

{
	...
	if( CNC_BitTest( instData->getStyle(), GWS_HORZ_SLIDER ) ) 
	{
		...
		if( CNC_BitTest( slider->winGetStatus(), WIN_STATUS_IMAGE ) )
			slider->winSetDrawFunc( getHorizontalSliderImageDrawFunc() );
		else
			slider->winSetDrawFunc( getHorizontalSliderDrawFunc() );

	}  // end if
	else if ( CNC_BitTest( instData->getStyle(), GWS_VERT_SLIDER ) ) 
	{
		...
		if( CNC_BitTest( slider->winGetStatus(), WIN_STATUS_IMAGE ) && !(parent && CNC_BitTest(parent->winGetStyle(), GWS_SCROLL_LISTBOX)))
	...
	if( CNC_BitTest( instData->getStyle(), GWS_MOUSE_TRACK ) )
																								  ...
																								  Bool defaultVisual )
{
  ...
  ComboBoxData *comboBoxData;
	...
	// we MUST have a push button style window to do this
	if( CNC_BitTest( instData->getStyle(), GWS_COMBO_BOX) == FALSE )
	{
...
// begin here
  ...
  comboBox->winSetUserData( comboBoxData );
	...
	// Set up list box redraw callbacks
	if( CNC_BitTest( comboBox->winGetStatus(), WIN_STATUS_IMAGE ))
	...
	if( CNC_BitTest( comboBox->winGetStyle(), GWS_MOUSE_TRACK ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( CNC_BitTest( instData->getStyle(), GWS_PROGRESS_BAR ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( CNC_BitTest( progressBar->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
  ...
  if( CNC_BitTest( instData->getStyle(), GWS_STATIC_TEXT ) ) 
	{
  ...
  if( textWin != NULL )
	{
		...
		if( CNC_BitTest( textWin->winGetStatus(), WIN_STATUS_IMAGE ) )
    ...
    memcpy( data, textData, sizeof(TextData) );
		...
		data->text->setWordWrapCentered( CNC_BitTest( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
...

{
	...
	if( CNC_BitTest( instData->getStyle(), GWS_ENTRY_FIELD ) == FALSE )
	{
	...
	if( CNC_BitTest( entry->winGetStatus(), WIN_STATUS_IMAGE ) )
																								 ...
																								 Bool assignVisual )
{
	...
	if( CNC_BitTest( instData->getStyle(), GWS_PUSH_BUTTON ) )
	{
	...
	else if( CNC_BitTest( instData->getStyle(), GWS_CHECK_BOX ) )
	{
	...
	else if( CNC_BitTest( instData->getStyle(), GWS_RADIO_BUTTON ) )
	{
	...
	else if( CNC_BitTest( instData->getStyle(), GWS_HORZ_SLIDER ) )
	{
	...
	else if( CNC_BitTest( instData->getStyle(), GWS_SCROLL_LISTBOX ) )
	{
	...
	else if( CNC_BitTest( instData->getStyle(), GWS_COMBO_BOX ) )
	{
//		ComboBoxData *comboBoxData = (ComboBoxData *)gadget->winGetUserData();
	...
	else if( CNC_BitTest( instData->getStyle(), GWS_PROGRESS_BAR ) )
	{
	...
	else if( CNC_BitTest( instData->getStyle(), GWS_STATIC_TEXT ) )
	{
	...
	else if( CNC_BitTest( instData->getStyle(), GWS_ENTRY_FIELD ) )
	{
...
GameWindow *GameWindowManager::getWindowUnderCursor( Int x, Int y, Bool ignoreEnabled )
{
	...
	else
	{
		...
		for( window = m_windowList; window; window = window->m_next )
		{
			...
			{
				if( CNC_BitTest( window->m_status, WIN_STATUS_ENABLED ) || ignoreEnabled )
				{
					// determine which child window the mouse is in
	...
	if( window )
	{
		...
		else if( ignoreEnabled && !( CNC_BitTest( window->m_status, WIN_STATUS_ENABLED ) ))
		{
			window = NULL;
		}
	}

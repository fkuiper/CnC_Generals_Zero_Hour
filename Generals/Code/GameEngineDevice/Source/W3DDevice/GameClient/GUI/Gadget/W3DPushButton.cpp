...
static void drawButtonText( GameWindow *window, WinInstanceData *instData )
{
	...
	text->setWordWrapCentered( CNC_BitTest( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
	text->setWordWrap(size.x);
	// get the right text color
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_ENABLED ) == FALSE )
	{
		textColor = window->winGetDisabledTextColor();
		dropColor = window->winGetDisabledTextBorderColor();
	}  // end if, disabled
	else if( CNC_BitTest( instData->getState(), WIN_STATE_HILITED ) )
	{
...
void W3DGadgetPushButtonDraw( GameWindow *window, WinInstanceData *instData )
{
	...
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_ENABLED ) == FALSE )
	{
		...
		if( CNC_BitTest( instData->getState(), WIN_STATE_SELECTED ) )
		{
																	 ...
																	 WinInstanceData *instData )
{
	...
	{
		if( CNC_BitTest( instData->getState(), WIN_STATUS_USE_OVERLAY_STATES ) )
		{
			ICoord2D size, start;
			// get window position
																	 ...
																	 WinInstanceData *instData )
{
	...
	if( !CNC_BitTest( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
	...
	if( image )
	{
		...
		{	
			...
			{
				if( !CNC_BitTest( window->winGetStatus(), WIN_STATUS_NOT_READY ) )
				{
					...
					if( !CNC_BitTest( window->winGetStatus(), WIN_STATUS_ALWAYS_COLOR ) )
					{
	...
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_FLASHING ) )
	{
	...
	if( CNC_BitTest( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
		...
		if( pushedOverlayIcon && hilitedOverlayIcon )
		{
			if(CNC_BitTest(window->winGetStatus(), WIN_STATUS_ENABLED))
			{
				if (CNC_BitTest( instData->getState(), WIN_STATE_HILITED ))
				{
					if( CNC_BitTest( instData->getState(), WIN_STATE_SELECTED ) )
					{
  			...
  			else if( CNC_BitTest( instData->getState(), WIN_STATE_SELECTED ) )
 				{

	...
	for( i = 0; i < numPolys; i++ )
	{
		...
		for( j = 0; j < MAX_POLYGON_NEIGHBORS; j++ )
		{
			...
			if( polyNeighbor->neighbor[ j ].neighborIndex != NO_NEIGHBOR )
			{
				...
				//
				if( CNC_BitTest( otherNeighbor->status, POLY_PROCESSED ) )
					continue;  // for j
			...
			//
			if( CNC_BitTest( polyNeighbor->status, POLY_VISIBLE ) )
			{
				...
				else if( CNC_BitTest( otherNeighbor->status, POLY_VISIBLE ) == FALSE )
				{
					...
					addSilhouetteEdge(meshIndex, polyNeighbor, otherNeighbor );
							 ...
							 CNC_BitTest( otherNeighbor->status, POLY_VISIBLE ) )
			{

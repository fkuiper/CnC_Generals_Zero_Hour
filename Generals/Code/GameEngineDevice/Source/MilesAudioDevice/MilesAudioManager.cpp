...
void MilesAudioManager::stopAudio( AudioAffect which )
{
	...
	PlayingAudio *playing = NULL;
	if (CNC_BitTest(which, AudioAffect_Sound)) {
		for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
			playing = *it;
			if (playing) {
	...
	if (CNC_BitTest(which, AudioAffect_Sound3D)) {
		for (it = m_playing3DSounds.begin(); it != m_playing3DSounds.end(); ++it) {
			playing = *it;
			if (playing) {
	...
	if (CNC_BitTest(which, AudioAffect_Speech | AudioAffect_Music)) {
		for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
			playing = *it;
			if (playing) {
				if (playing->m_audioEventRTS->getAudioEventInfo()->m_soundType == AT_Music) {
					if (!CNC_BitTest(which, AudioAffect_Music)) {
						continue;
					}
				} else {
					if (!CNC_BitTest(which, AudioAffect_Speech)) {
						continue;
					}
				}
		...
		else 
		{
			...
			if (pos) 
			{
				...
				else
				{
					...
					Bool playAnyways = CNC_BitTest( playing->m_audioEventRTS->getAudioEventInfo()->m_type, ST_GLOBAL) || playing->m_audioEventRTS->getAudioEventInfo()->m_priority == AP_CRITICAL;
